...
resource "aws_lambda_function" "test_lambda1" {
  filename         = "challenges/lambda-env/data/lambda/lambda_function.zip"
  ...
  environment {
    variables = {
      RDS_USER = "admin",
      RDS_PASSWORD = random_password.database-secret.result
    }
  }
}
...
resource "aws_lambda_function_url" "lambda1-url" {
